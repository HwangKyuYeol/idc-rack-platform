...
function DeviceArt({kind,imageUrl}){
  ...
  return <div className="art server"><Server size={20}/></div>;
}
...
function App(){
  ...
  function addDevice(){
    ...
    while(s>0){
      let ok=true;
      for(let i=0;i<selected.u;i++) if(occupied.has(s-i)||s-i<1) ok=false;
  ...
  return <div className="page">
    <header><div><h1>IDC Rack Power Platform v2</h1><p>실제 사진 URL · 전력 자동 추천 AI · A/B 전원 · 보고서 출력</p></div><div className="actions"><button onClick={exportPng}><ImageIcon size={16}/>PNG</button><button onClick={exportPdf}><Download size={16}/>PDF</button><button className="outline" onClick={exportCsv}><FileText size={16}/>CSV</button></div></header>
    <main>
      ...
      <aside>
        ...
        <section className="panel"><h2>장비 목록</h2><button className="outline small" onClick={()=>setItems([])}>초기화</button><div className="list">{[...items].sort((a,b)=>b.startU-a.startU).map(x=><div className="listItem" key={x.uid}><div><b>{x.startU}U · {x.brand} {x.model}</b><br/><span>{x.type} · {x.u}U · avg {x.avg}W · max {x.max}W</span></div><button className="delete" onClick={()=>setItems(p=>p.filter(y=>y.uid!==x.uid))}><Trash2 size={15}/></button></div>)}</div>{groups.length>0&&<div className="info"><b>자동 이중화 후보</b><br/>{groups.map(([k,v])=>`${k} (${v})`).join(' / ')}</div>}</section>
      </aside>
    </main>
  </div>
}
